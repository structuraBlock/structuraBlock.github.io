...
class ArmorStandGeo {
    ...
    constructor(name: string, alpha: number = 0.8, offsets: number[] = [0, 0, 0], size: number[] = [64, 64, 64], refPack: string = "Vanilla_Resource_Pack") {
    ...
    exportBig(packFolder: string): void {
    ...
    makeBlock(x: number, y: number, z: number, blockName: string, rot: number | null = null, top: boolean = false, data: number = 0, trapOpen: boolean = false, parent: string | null = null, variant: string = 'default', big: boolean = false): void {
    ...
    extendUVImage(newImageFilename: string): void {
    ...
    blockNameToUV(blockName: string, variant: string = '', shapeVariant: string = 'default', index: number = 0, data: number = 0): any {
        let tempUV = {};
    ...
    addBlocksToBones(): void {
        for (let key in this.blocks) {
            this.geometry['bones'].push(this.blocks[key]);
        }
    }

    getBlockTexturePaths(blockName: string, variant: string = ''): any {
