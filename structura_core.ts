...
import { PathLike, createWriteStream, mkdirSync, readFileSync } from 'fs';
import { copyFileSync, removeSync } from 'fs-extra';
import { time } from 'console';
import archiver from 'archiver';


function createZipArchive(sourceDir: string, outputZipPath: PathLike) {
...
export class structura {
    ...
    pack_name: string;
    structure_files: { [key: string]: { file: string, offsets: number[] } };
    rc: rcc.RenderController;
    ...
    all_blocks: { [key: string]: any };
    icon: string;
    dead_blocks: { [key: string]: { [key: string]: number } };

    constructor(pack_name: string) {
        mkdirSync(pack_name);
    ...
    add_model(name: string, file_name: string) {
        this.structure_files[name] = { file: file_name, offsets: null };
    }

    set_model_offset(name: string, offset: number[]) {
