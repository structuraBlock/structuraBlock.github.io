...
const buffer = await readFile("../test_structures/1-19 blocks.mcstructure");
...
const NBTData = {
    ...
    while (proto !== null) {
      for (let prop in proto) {
        if (typeof proto[prop] === 'function') {
          console.log(`Method (from ${proto.constructor.name}): ${prop}`);
        } else {
          console.log(`Property (from ${proto.constructor.name}): ${prop} = ${proto[prop]}`);
        }
      }
  ...
  console.log((data.data.structure.block_indices))
  console.log(Object.prototype.toString.call(  data.data.structure.block_indices[0][0]))
  // {
  //   data: {
  //     format_version: 1,
  //     size: [ 14, 6, 6 ],
